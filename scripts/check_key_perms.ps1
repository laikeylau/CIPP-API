...
foreach ($T in $Tenants) {
    ...
    try {
        ...
        # Check key Graph roles
        $AssignedIds = $Roles | ForEach-Object { $_.appRoleId }
        foreach ($RoleId in $KeyRoles.Keys) {
            $Status = if ($RoleId -in $AssignedIds) { "OK" } else { "MISSING" }
            Write-Host "  $Status : $($KeyRoles[$RoleId])"
        }
        
        # Check EXO roles
        $ExoAppId = "00000002-0000-0ff1-ce00-000000000000"
        $ExoRoles = $Roles | Where-Object { $_.resourceId -match $ExoAppId }
        if ($ExoRoles) {
            Write-Host "  EXO roles: $($ExoRoles.Count)"
        } else {
            # Try by resourceDisplayName
            $ExoRoles2 = $Roles | Where-Object { $_.resourceDisplayName -match "Exchange" }
            Write-Host "  EXO roles (by name): $($ExoRoles2.Count)"
        }
        
        # Check for Directory.Read.All specifically (needed for LicenseOverview)
        $DirReadAll = $Roles | Where-Object { $_.appRoleId -eq "bf394be3-5869-4435-9473-b8b2a3e30038" }
        if (-not $DirReadAll) {
            Write-Host "  *** NEEDS Directory.Read.All consent ***"
        }
